...
module Step03_Sequences =
  ...
  let run () =
    ...
    // Challenge 3 --------------------------------------
    let aList = [7; 11; 13; 17]

    ensure (aList = [7; 11; 13; 17])

    // Challenge 4 --------------------------------------
    let aSet = Set.ofList [1; 2; 3; 7]

    ensure (aSet = Set.ofList [1; 2; 3; 7])

    // Challenge 5 --------------------------------------
    let aSeq = Seq.ofList [99; 100; 103]
    ...
    ensure (List.head [42] = 42)

    // Challenge 7 --------------------------------------
    // Similar story with `tail`
    ensure (List.tail [1; 2; 3; 4] = [2; 3; 4])
    ...
    ensure (r1 = ["hi"; "howdy"; "hello"])

    // Challenge 9 --------------------------------------
    // Try it with an unnamed function (hint "not equals" is `<>` in F#)
    let noYo = List.filter (fun x -> x <> "yo") ["hi"; "howdy"; "yo"; "sup"; "hello"]
    ...
    let noYo = List.filter ((<>) "yo") ["hi"; "howdy"; "yo"; "sup"; "hello"]

    ensure (noYo = noYoExpected)

    // Challenge 11 --------------------------------------
    // `map` transforms all values in a container making a new container
    let allIncremented = List.map (fun x -> x + 1) [0; 1; 2; 3]
    ...
    *)
    ...
    ensure (exclaimOnlyStartsWithH  = ["hi!"; "howdy!"; "hello!"])
    ...
    *)
    ...
    let totalStringLen = List.fold (fun result aStr -> result + stringLen aStr) 0 ["good"; "day"; "mate"]
